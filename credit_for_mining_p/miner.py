...
"""
if os.path.isfile('my_id.py'):
    print("Id file already exists")
    f = open(".my_id.py", "r")
    recip_id = f.read()
    f.close()

elif not os.path.isfile('my_id.py'):

    recip_id = str(uuid4()).replace('-', '')
    print(f"Created Id file, now using uuid: {recip_id}")
    ...
    while True:
        ...
        if new_proof:
            ...
            if data['message'] == "New Block Forged":
                ...
                coins_mined += 1
                print(f"Congratulations! {data['message']} \nCoins mined: {coins_mined}")
            else:
                print(f"Failure! Nothing mined. {data['message']}")
